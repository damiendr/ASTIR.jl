...
program!
"""
function raise_flow(i, flow::FlowGraph, seen=Set(Int[]), while_start=-1, while_end=-1)
    dest = []
    ...
    while 0 < i <= length(flow.statements)
        ...
        if isa(stmt, GotoNode)
            target = stmt.label
            if target == while_start
                push!(seen, i)
                push!(dest, Expr(:continue))
                return dest, -1
            elseif target-1 == while_end
                push!(seen, i)
...
#            println("$i: 'if' $test $else_target")
    ...
    #            println("$i: 'if' reconverged at $if_tail/$else_tail")
